...
visNetwork(Contributor_nodes, Contributor_edges_plot, width = "100%", height = "800px") %>%
  visIgraphLayout(layout = "layout_nicely", randomSeed = 1001) %>%
  visPhysics(
    enabled = TRUE,
    stabilization = list(
      enabled = TRUE,
    ...
    forceAtlas2Based = list(
      ...
      centralGravity = 0.01,         # Weaker central pull
      springLength = 100,            # Longer springs
  ...
  visNodes(
    font = list(size = 5, strokeWidth = 1, strokeColor = "white"),
    size = 7,
    borderWidth = 0,
    color = list(background = "#353839", highlight = list(background = "#A52828", border = "darkred")),
  ...
  htmlwidgets::onRender("
    function(el, x) {
      var network = this;
      // Add this to set initial scale
      network.on('initRedraw', function() {
        network.moveTo({
          scale: 0.1,  // initial zoom level
          position: {x:0, y:0}  // Center position
        });
      });
      // Add this to maintain scale after stabilization
      network.on('stabilized', function() {
        network.moveTo({
          scale: 0.1,
          animation: false  // Disable animation for instant repositioning
        });
      });
